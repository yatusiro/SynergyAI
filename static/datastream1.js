...
var headbeta=head;
  ...
  function add_func(){
    ...
    addnode(capsID,orID,333+x,136+y);
  }
  function delete_func(){
    //var str = prompt("delet item");
    
        if(cy.elements()[0]._private.data.id){
            
            var name = cy.elements()[0]._private.data.id;
  ...
  function addnode(Name,Nameor,x,y){
    cy.add({
    ...
    cy.add({
        classes: 'automove-viewport',
        data: {id: Nameor, parent: Name, label: 'data(id)', type: '' + nodeDrawType},
        classes: 'CSV-Image',
        position: {
            x: x,
            y: y
