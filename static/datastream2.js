...
//window.location.href='https://blog.csdn.net/qq_40660389/article/details/125312395';
//readcsv_JIANG
...
var headbeta=head;
  ...
  function addnode(parent,Name,Nameor,x,y){
    cy.add({
        classes: 'automove-viewport',
        data: {id: Name, type: 'CAPSULE'},
    ...
    cy.add({
        classes: 'automove-viewport',
        data: {id: Nameor, parent: parent, label: 'data(id)', type: '' + nodeDrawType},
        classes: 'CSV-Image',
        position: {
            x: x,
            y: y
...
function opennewpage(){
    ...
    if(confirm(msg)==true){
        //window.location.href='http://192.168.0.70:7777/index3';
        window.location.href='http://192.168.108.14:7777/index3';
    }else{}
    //window.location.href='https://blog.csdn.net/qq_40660389/article/details/125312395';
}
//下拉框添加成员  
...
function closeWin() {
    console.log(document.getElementById('nodeName').innerHTML);
        ...
        style: [
            ...
            
            {
                ...
                style: {
                    ...
                    'label': 'data(id)',
                    'shape': 'round-rectangle',
                    'background-image': 'static/img/csv.png',
                    'background-opacity': 0,    /*make background disappear but reserve img*/
                    /*make img fit to node*/
                    'background-fit': 'contain'
                }
...

            {
                ...
                select: function (ele) {
                    ...
                    if(ele.data().csv_lier){
                        console.log("ele.id():" + ele.id());
                        console.log(ele.data().csv_data);
                        // openIt();
                        var lier=ele.data().csv_lier;
                        console.log(lier);
                        let column = lier;
                        if(ele.data().csv_lier!=undefined){
                            lier=changelier(lier);
                        }
                        else{
                            alert("lier undefined!");
                        }
                        if(lier!=column){
                            ele.data().csv_lier = lier;
                            console.log("now is "+ele.data().csv_lier);

                            data = select(temp,lier);
                            const strdata = JSON.stringify(data);
                            ele.data().csv_lier = lier;
                            ele.data().csv_data = strdata;
                        }
                    }
                    else{
                        alert("This is not a csv item!");
                    }
...
document.addEventListener('DOMContentLoaded', function () {
    let a = Math.round(Math.random() * 3000);
    let jsonUrl
    if(a<1000){
         jsonUrl = 'static/data/data_after.json';
    }
    else if(a>1000 && a<2000){
         jsonUrl = 'static/data/data_after2.json';
    }
    else if(a<3000 && a>2000){
         jsonUrl = 'static/data/data_after3.json';
    }
    else{
         jsonUrl = 'static/data/data_after.json';
    }
...
// }
function changelier(lier){
    var str = prompt("change lier");
    for(a=0;a<document.mylist.myselect.length;a++){
        console.log(mylist.myselect.options[a].text);
        if(str==document.mylist.myselect.options[a].text){
            if(str!=lier){
                    alert("lier changed");
                    console.log(str);
                    return str;
...
const data = tf.data.csv(c);
const sample = data.take(10);
...
function readcsvbeta(){
    var value = document.mycsv.selectcsv.value;
    return value;
